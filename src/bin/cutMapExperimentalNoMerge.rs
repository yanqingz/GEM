...
fn main() {
    ...
    let stages_effective_parts = stageds.iter().map(|&(l, r, ref staged)| {
        ...
        clilog::info!("Adaptive partitioning completed. Building final original partitions without merging.");
        
        // Build final original partitions without merging
        let final_original_parts = {
            clilog::info!("Building final original partitions in parallel (input: {} partition indices)", best_parts_indices.len());
            best_parts_indices.par_iter().enumerate().map(|(i, v)| {
                let part = Partition::build_one(&aig, staged, v);
                if part.is_none() {
                    clilog::error!("Partition {} exceeds resource constraint.", i);
                }
        ...
        // Filter out None values and unwrap the Some values
        let effective_parts: Vec<Partition> = final_original_parts.into_iter()
            .filter_map(|part_opt| part_opt)
            .collect();
            
        let failed_count = best_parts_indices.len() - effective_parts.len();
        if failed_count > 0 {
            clilog::warn!("{} partitions failed to build and were filtered out", failed_count);
        }
        clilog::info!("built {} original partitions (no merging).", effective_parts.len());
        effective_parts
    }).collect::<Vec<_>>();
